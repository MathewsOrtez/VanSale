...
const TaxTable: React.FC = () => {
  ...
  const currentData = taxData.slice(startIndex, endIndex);
...
const handleDeleteSelected = () => {
 if(selectedRows.length > 0){
  ...
  // Open the modal
  const handleClickOpen = (srl: number ,name: string, shortname: string, active: boolean) => {
  ...
  const handleEditClick = (srl: number ,name: string, shortname: string, active: boolean) => {
  ...
  const handleSave = (updatedData: any) => {
    editTax(updatedData);
    CustomAlert({ type: "success" ,message: `${updatedData.name} updated successfully.` });
  ...
  return (
    ...
    <div className="mx-4">
      <div className="flex justify-between items-center  my-7">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-4 ">
              ...
              {/* Add New Button */}
              <button className=" w-[98px] h-[33px] bg-[#25add7] rounded-md text-white text-md font-semibold hover:bg-[#1a8d9d] transition-colors"
      ...
      <table className={`w-full  border-collapse rounded-md ${openModal ? "blur-sm" : ""} ${editModal ? "blur-sm" : ""}  ${openNewModal ? "blur-sm" : ""}`}>
        <thead className="bg-gray-100 border text-sm">
          <tr>
          <th className="py-1 text-gray-500 font-semibold text-start "><CustomizedCheckbox checked={selectAll}
                onChange={(e) => handleHeaderCheckboxChange(e.target.checked)}/></th>
          <th className="py-1 text-gray-500 font-semibold text-start" >Srl</th>
          <th className="py-1 text-gray-500 font-semibold text-start" >Name</th>
          <th className="py-1 text-gray-500 font-semibold text-start" >Short Name</th>
          <th className="py-1 text-gray-500 font-semibold text-center" >Active</th>
          <th className="py-1 text-gray-500 font-semibold text-center" >Edit</th>
          <th className="py-1 text-gray-500 font-semibold text-center" >Delete</th>
          </tr>
        </thead>
        ...
        {currentData.length === 0 ? (
          ...
          currentData.map((item : any, index : number) => (
            ...
            >
              <td className="py-2">
                <CustomizedCheckbox checked={selectedRows.includes(item.srl)}
                onChange={(e) =>
                  handleRowCheckboxChange(item.srl, e.target.checked)}/>
              </td>
              <td className="py-2"  onClick={()=> handleClickOpen(item.srl,item.name,item.shortname,item.active)}>{item.srl}</td>
              <td className="py-2"  onClick={()=> handleClickOpen(item.srl,item.name,item.shortname,item.active)}>{item.name}</td>
              <td className="py-2"  onClick={()=> handleClickOpen(item.srl,item.name,item.shortname,item.active)}>{item.shortname}</td>
      ...
      {openNewModal && <TaxAddNew open={openNewModal} onClose={handleNewClose} onSave={handleSave} />}
      {openModal && <DeleteTaxModal open={openModal} onClose={handleClose} deleteData={deleteData} onSave={handleSave} onDelete={handleDelete} />}
      {editModal && (
        <EditTaxModal open={editModal} onClose={handleEditClose} editData={editData} onSave={handleSave} />
      )}
